...
**Consequences.**
- [x] Trivial local install  -  one `bun run dev` and you have
  everything.
- [x] The hash chain is consistent by construction (single writer).
- [x] Deploying a small-team install is a single container.
- WARN: The pipeline runner is synchronous to its HTTP request; long
...
**Consequences.**
- [x] Swapping providers is one env var.
- [x] New providers (Groq, Together, Anyscale, ...) are a copy of
  `openrouter.ts` away.
- [x] The mock adapter is deterministic  -  UI screenshots are stable.
- [x] A bad `AI_ADAPTER` value falls back to mock, never to a paid
  adapter.
- WARN: The interface is intentionally narrow (4 methods). More
...
**Consequences.**
- [x] Tamper-evidence is a 30-line implementation.
- [x] An external auditor with read access can reproduce the chain.
- [x] The chain survives database migrations because it's computed
  over content, not row IDs alone.
- WARN: The chain is **global**  -  splitting it per-case would lose
  cross-workflow attestation; we judged that loss bigger than the
  cost of a single linear replay.
- WARN: We use SHA-256, not a Merkle tree. Linear replay is enough
...
**Consequences.**
- [x] Zero environment setup for new contributors.
- [x] The same migrations run against both databases.
- WARN: Some Postgres-specific features (`JSONB`, advisory locks,
...
**Consequences.**
- [x] Tiny bundle, no boilerplate.
- [x] Every mutation declares exactly which queries it invalidates.
- [x] `persist` middleware keeps `activeView` + sidebar collapse
  across page reloads.
- WARN: Persisted state has to be migrated by hand when the store
...
**Consequences.**
- [x] Instant view switching  -  no route loading.
- [x] The command palette is trivial to build (no router push).
- [x] Screenshots can address every view by URL.
- WARN: Server-rendered SEO is sacrificed  -  that's fine; this app is
...
**Consequences.**
- [x] The OSS deployment never touches billing code.
- [x] Audit logs in OSS look identical to audit logs in SaaS  -  so
  forensic exports are portable.
- [x] Premium feature graveyards (Shodan, Censys, Hunter, PDF
  export, multi-tenant org isolation, ClaudeAdapter) are easy to
  add without touching core paths.
- WARN: The OSS deployment cannot use ClaudeAdapter (by design).
...
**Consequences.**
- [x] Engineers can see the source design without context-switching
  repositories.
- [x] The merge rationale is auditable.
- WARN: The reference dirs add ~4 MB to a fresh clone. Tsconfig +
...
**Consequences.**
- [x] Consistent visual language across 15 views.
- [x] Hash display is at-a-glance recognisable.
- WARN: No light theme  -  that's a Phase 8 polish item, not a P1.
...
**Consequences.**
- [x] Zero new runtime dependencies for graph rendering.
- [x] Deterministic = screenshot-stable.
- [x] Pure SVG works perfectly in PDF exports.
- WARN: No drag-to-reposition, no zoom/pan, no live physics. When
...
- [x] App routes still benefit from the `server-only` marker.
- WARN: Two files instead of one. Small price for the clean
  boundary.
