...
| LLM adapter | `openrouter` (with the deployment's configured model) |
...
| Entity relations | inspected via the entity graph; relation count was non-zero but is not the primary measure |
| Audit-log entries | 11 (pipeline start, per-agent-group completion, entity extraction, report generation, bridge-to-case, plus the prior failed-run sequence preserved in the chain) |
...
`identity` agent group's output for this run was flagged
`unverified` because the LLM produced prose rather than a
structured finding. The platform refused to upgrade the
confidence, persisted the rejection, and surfaced the issue to
the operator. This is the SAT-discipline guardrail working as
designed (see [`../10-ANALYTIC_FRAMEWORK.md`](../10-ANALYTIC_FRAMEWORK.md)
section 4): the platform prefers to flag epistemic weakness
explicitly rather than fabricate structure.
