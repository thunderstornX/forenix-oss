...
|---|---|---|
| FR-01 | Create investigations with target, target type, agent groups, objective | 1 [x] |
| FR-02 | Run a configurable OSINT pipeline driven by an AI adapter | 1+3 [x] |
| FR-03 | Persist findings with confidence, priority, source, reasoning trace | 1 [x] |
| FR-04 | Auto-extract entities + relations from findings | 3 [x] |
| FR-05 | Promote a finding to forensic evidence in one click | 2 [x] |
| FR-06 | Bridge an investigation to a forensic case | 2 [x] |
| FR-07 | Evidence with Git-style branches + commits | 1 [x] |
| FR-08 | Hash-chained audit log across both workflows | 1 [x] |
| FR-09 | Integrity verification: replay the chain, surface tampering | 1 [x] |
| FR-10 | Pluggable AI adapter (mock / local / hosted / sovereign / SaaS-paid) | 1+7 [x] |
| FR-11 | Cross-case knowledge graph | 5 [x] |
| FR-12 | Reports (sectioned JSON + markdown) | 6 [x] |
| FR-13 | AI Lab  -  visibility into agent input/output/confidence | 6 [x] |
| FR-14 | Monitors  -  scheduled re-runs (record-only in 0.1) | 6 [x] |
| FR-15 | Verification  -  claim-level verdicts | 6 [x] |
| FR-16 | Merge-request reviews on evidence branches | 4 [x] (data + list view) |
| FR-17 | Command palette ⌘K | this pass [x] |
